...
class Hybrid_Providers_Soundcloud extends Hybrid_Provider_Model {
	...
	function initialize() {
		...
		$path = $this->config["path_libraries"] ? : Hybrid_Auth::$config["path_libraries"] . 'Soundcloud/';
	...
	function loginFinish() {
		...
		try {
			$response = $this->api->accessToken($token, array(), array(
	...
	function getUserProfile() {
		...
		try {
			$response = json_decode($this->api->get("me", array(), array(
						CURLOPT_SSL_VERIFYPEER => false,
						CURLOPT_SSL_VERIFYHOST => false,
			)));
